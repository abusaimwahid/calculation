...
export default async function Dashboard() {
  const [tx, accounts, assets, debts, holdings, flows, settings] = await Promise.all([
    prisma.transaction.findMany({ orderBy: [{ date: "desc" }, { createdAt: "desc" }], include: { account: true } }),
  ...
  const assetsTotal = assets.reduce((s, a) => s + Number(a.currentValue ?? a.purchaseValue), 0);
  ...
  const netWorth = cash + assetsTotal + cryptoBdt + receivable - payable;
  ...
  return <>
    ...
    <section className="kpiGrid">
      ...
      <Kpi label="This month expense" value={money(monthExpense)} note={`Income ${money(monthIncome)}`} tone={monthExpense > monthIncome && monthIncome > 0 ? "bad" : "default"} />
      ...
      <Kpi label="Assets" value={money(assetsTotal)} note={`${assets.length} tracked items`} />
      <Kpi label="Payable debts" value={money(payable)} note={`Receivable ${money(receivable)}`} tone={payable > 0 ? "bad" : "good"} />
    </section>
    ...
    <div className="twoCol">
      <section className="panel"><div className="panelHead"><div><h2>Recent transactions</h2><p>Latest daily records</p></div><Link href="/transactions">View all</Link></div>
        <div className="compactList">{tx.slice(0,7).map(t=><div key={t.id} className="compactRow"><div><b>{t.category}</b><small>{shortDate(t.date)}{t.note ? ` · ${t.note}` : ""}</small></div><strong className={t.type === "INCOME" ? "positive" : "negative"}>{t.type === "INCOME" ? "+" : "−"}{money(Number(t.amount))}</strong></div>)}</div>
      </section>
